...
class SigmaTauSearchExperimentViolation:
    ...
    def run(self):
        for animal in self.animals:
            ...
            dmg = DesignMatrixGeneratorViolation(verbose=False)
            ...
            for sigma in self.sigmas:
                for idx, tau in enumerate(self.taus):
                    ...
                    print(
                        f"sigma: {sigma}, tau: {tau}, size of X: {X.shape}, Y: {Y.shape}"
                    )
                    (
                        X_train,
                        X_test,
                    ...
                    #! consider storing the test sessions and train sessions (as well as n trials in each?)

        self.results = pd.concat(self.stored_fits, ignore_index=True)
        return self.results

    def store(self, animal, model_name, nll, sigma, tau, X, W_fit, Y_train, dmg):
        ...
        iter_df = pd.DataFrame(
            {
                ...
                "n_test_trials": [len(X)],
                "test_sessions": [dmg.test_sessions],
                "n_train_trials": [len(Y_train)],
                "train_sessions": [dmg.train_sessions],
            }
        )
...
class DesignMatrixGeneratorViolation:
