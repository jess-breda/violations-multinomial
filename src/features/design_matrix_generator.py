...
class DesignMatrixGenerator:
    ...
    def generate_base_matrix(
        self, df, model_type="multi", psytrack_features=False, return_labels=True
    ):
        """
...
class DesignMatrixGeneratorViolationHistory(DesignMatrixGenerator):
    ...
    def generate_design_matrix(self, df, filter_params, no_prev_violation=False):
        """
        Function to generate a base design matrix with an
        optional exponential filter on the previous violation
        column.
        ...
        df : pd.DataFrame
            ...
            get_rat_viol_data() or get_rat_data()
        filter_params : dict
            dictionary with {"prev_violation": tau} where tau is the
            time constant for exponential filter, if tau is 0 or None
            then no filtering is applied and prev_violation remains
            as a binary encoding
        no_prev_violation = bool (default=False)
            whether to drop the prev_violation column from the design matrix
            so there is no history at all- this is used for model
        ...
        X  : pd.DataFrame (N x D)
            ...
            `prev_correct_side` `prev_choice`, `prev_violation_exp` and
            `session` for train/test split

        y : np.array (N x C) if multi, N x 1 if binary
            labels for design matrix, where C is the number of classes

        """
        # this contains a "prev_violation" column
        X, y = super().generate_base_matrix(df, return_labels=True)

        tau = filter_params["prev_violation"]
        if no_prev_violation:
            X.drop(columns=["prev_violation"], inplace=True)
        elif tau:
            X = super().exp_filter_column(X, tau=tau, column="prev_violation")
...
class DesignMatrixGeneratorInteractions(DesignMatrixGenerator):
    ...
    def generate_design_matrix(self, df, tau, filter_column, interaction_pairs):
        ...
        if tau:
            X = super().exp_filter_column(X, tau=tau, column=filter_column)
...
class DesignMatrixGeneratorFilteredHistory(DesignMatrixGenerator):
    ...
    def generate_design_matrix(self, df, filter_params, interaction_pairs=None):
        ...
        for column, tau in filter_params.items():
            X = super().exp_filter_column(X, tau=tau, column=column)
